...
export class ActiveBlockService {
  ...
  select(definition: BlockDefinition | PlaceableItemDefinition): void {
    ...
    this.active.set({ id, ...(item && definition.itemId !== id ? { itemId: definition.itemId } : {}), ...(item && definition.placementKind !== 'direct' ? { placementKind: definition.placementKind } : {}), state: { ...definition.defaultState }, support: definition.support });
  }
