...
function manualVisualObservations(report: VanillaAssetCoverageReport): string {
  ...
  return [`- Slabs: ${family('slabs')}; \`minecraft:stone_slab\` is ${describe('minecraft:stone_slab')}.`, `- Stairs: ${family('stairs')}; \`minecraft:oak_stairs\` is ${describe('minecraft:oak_stairs')}.`, `- Bed: \`minecraft:red_bed\` is ${describe('minecraft:red_bed')}.`, `- Trapdoor: \`minecraft:oak_trapdoor\` is ${describe('minecraft:oak_trapdoor')}.`, `- Wall Torch: ${describe('minecraft:wall_torch')}.`, `- Fence: ${describe('minecraft:oak_fence')}; connection refresh remains a behavior issue.`, `- Door: ${describe('minecraft:oak_door')}; placement failure remains a behavior issue.`, `- Sunflower: ${describe('minecraft:sunflower')}; placement failure remains a behavior issue.`, `- Dandelion: ${describe('minecraft:dandelion')}.`, `- Glass Pane: ${describe('minecraft:glass_pane')}.`, `- Wall: ${describe('minecraft:cobblestone_wall')}.`].join('\n');
}
function record(value: unknown): Record<string, unknown> { return typeof value === 'object' && value !== null && !Array.isArray(value) ? value as Record<string, unknown> : {}; }
function disposeObject(object: THREE.Object3D): void { object.traverse((child) => { if (!(child instanceof THREE.Mesh)) return; child.geometry.dispose(); const materials = Array.isArray(child.material) ? child.material : [child.material]; for (const material of materials) { if (material.map?.userData['ownedBedAtlasTexture']) material.map.dispose(); material.dispose(); } }); }
