...
export class IndexedDbAssetCache {
  async load(): Promise<SerializedVanillaAssets | undefined> {
    ...
    const stored = await request<CachedVanillaAssets | undefined>(database, STORE_NAME, 'readonly', (store) => store.get(VANILLA_ASSET_VERSION));
    if (!stored) return undefined;
    const bundle = migrateCachedVanillaAssets(stored);
    if (stored.schemaVersion !== bundle.schemaVersion) await request(database, STORE_NAME, 'readwrite', (store) => store.put({ ...bundle, id: VANILLA_ASSET_VERSION }));
    return bundle;
  }

  async save(bundle: SerializedVanillaAssets): Promise<void> {
    const database = await openDatabase();
    await request(database, STORE_NAME, 'readwrite', (store) => store.put({ ...bundle, id: VANILLA_ASSET_VERSION }));
  }
