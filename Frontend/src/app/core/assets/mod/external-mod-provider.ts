...
export class ExternalModProvider implements ContentSourceProvider {
  ...
  catalog(): BlockCatalogSource & { readonly paintingVariants: readonly PaintingVariant[] } {
    ...
    for (const path of blockstatePaths) {
      ...
      const initial: AssetBlockRecord = { id, displayName: typeof language[`block.${namespace}.${blockPath.replaceAll('/', '.')}`] === 'string' ? language[`block.${namespace}.${blockPath.replaceAll('/', '.')}`] as string : humanize(blockPath), defaultState: {}, stateDefinitions: definitions, resources: { blockstate: path, model: configuredModelIds(blockstate)[0], textures: [] }, support: 'partial', visualSupport: 'partial', behaviorSupport: 'unknown', defaultStateSource: 'unknown', visualClassification: 'standard-json', visualClassificationEvidence: 'inferred', sourceId: this.source.id, sourceName: this.source.displayName, modName: this.source.displayName, trustedBehaviorFamilies: trustedFamilies, semanticEvidence: trustedFamilies.map((contractId) => ({ contractId, provenance: 'trusted-data' as const, strength: 'partial' as const, supportingTags: trustedTagIdsFor(id, tagIndex), supportingProperties: [], supportingResources: [] })), behaviorEvidenceRequired: true };
      ...
      records.push({ ...withState, itemEvidence: matchingItem ? { itemId: id, placeable: true, sourceFormat: matchingItem.sourceFormat, referencedModels: matchingItem.referencedModels, referencedResources: matchingItem.referencedResources } : undefined, contentDescriptor: introspection.inspectBlock(withState) });
    }
