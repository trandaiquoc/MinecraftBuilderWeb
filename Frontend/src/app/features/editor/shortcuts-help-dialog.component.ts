...
import { KEYBOARD_ACTIONS, KeyboardAction } from '../../core/editor/keyboard-bindings';
import { MOUSE_ACTIONS, MouseAction, mouseBindingTokens } from '../../core/editor/mouse-bindings';
...
export class ShortcutsHelpDialogComponent {
  ...
  private displayToken(token: string): string { return token.replace('LeftClick', this.i18n.t('mouseLeftClick')).replace('RightClick', this.i18n.t('mouseRightClick')).replace('MiddleClick', this.i18n.t('mouseMiddleClick')).replace('WheelUp', this.i18n.t('mouseWheelUp')).replace('WheelDown', this.i18n.t('mouseWheelDown')); }
}
