...
describe('EditorStatusBarComponent asset bootstrap status', () => {
  it('renders determinate Mod restore progress and removes it when ready', async () => {
    ...
    const downloadProgress = fixture.nativeElement.querySelector('.asset-progress') as HTMLProgressElement | null;
    expect(downloadProgress?.value).toBe(42);
    expect(downloadProgress?.max).toBe(100);
    ...
    const progress = fixture.nativeElement.querySelector('.asset-progress') as HTMLProgressElement | null;
    expect(progress?.value).toBe(1);
    expect(progress?.max).toBe(2);

    assets.contentRestore.set({ phase: 'ready', current: 2, total: 2, failed: 0 });
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelector('.asset-progress')).toBeNull();
  ...
  it('renders indeterminate loading and partial warning states without a loading bar when complete', async () => {
    ...
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelector('.asset-progress')).not.toBeNull();
    expect((fixture.nativeElement.querySelector('.asset-progress') as HTMLProgressElement).value).toBe(0);
