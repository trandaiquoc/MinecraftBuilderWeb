...
export class ViewportComponent implements AfterViewInit, OnDestroy {
  ...
  private readonly assetSync = effect(() => { this.engine.setVisualProvider(this.assets.visualProvider()); this.engine.setSpecialVisualDescriptorResolver((id) => this.library.get(id)?.specialVisual); this.engine.setBlockDefinitionResolver((id) => this.library.get(id)); this.engine.setDecorationTextureProvider((resource) => this.assets.sources.resources.textureUrl(resource)); this.engine.setDecorationItemResourceProvider((itemId) => itemVisualTextureResources(this.assets.sources.resources, itemId)); this.engine.setDecorationItemVisualProvider((itemId) => resolveItemVisual(this.assets.sources.resources, itemId)); this.paintingCatalog.variants(); this.engine.setPaintingTextureResolver((id) => this.paintingCatalog.get(id)?.assetPath); });
