...
Content discovery is independent: blockstate JSON is Block evidence, `assets/<namespace>/items/**/*.json` is authoritative Item evidence, and legacy `models/item/**/*.json` is promoted only when the model is an item root (`item/generated` or `item/handheld`) or another retained registry/language/tag/recipe/loot/predicate source proves the Item identity. Helper models remain available to the model graph but do not become selectable Items by filename alone. `data/<namespace>/painting_variant/**/*.json` plus the standard placeable tag are Painting evidence. An item-only resource never becomes a placeable Block. External block behavior is only shared with Vanilla contracts when trusted standard tag evidence and state/resource shape agree; name suffixes are not behavior metadata.
